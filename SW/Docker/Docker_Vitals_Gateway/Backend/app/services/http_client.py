...
def login_access_token(user_name, password):
    url = f"{environment.BASE_URL}/login/access-token"
...
def get_facility_list(skip=0, limit=100):
    url = f"{environment.BASE_URL}/facility/list?skip={skip}&limit={limit}"
...
def get_device_list(skip=0, limit=100):
    url = f"{environment.BASE_URL}/device/list?skip={skip}&limit={limit}"
...
def register_tcp_server(facility_id, unique_name):
    url = f"{environment.BASE_URL}/facility/register-tcp-server"
...
async def register_device(device_id):
    url = f"{environment.BASE_URL}/device/create"
