...
def update_environment_file(new_file_path, file_type):
    ...
    path_for_env = new_file_path.replace("\\", "/")
    ...
    for line in lines:
        if file_type == "root_ca" and line.startswith("root_ca_path"):
            updated_lines.append(f'root_ca_path = "{path_for_env}"\n')
            r.set("root_ca_path", new_file_path.replace("\\", "/"))
        elif file_type == "private_key" and line.startswith("private_key_path"):
            updated_lines.append(f'private_key_path = "{path_for_env}"\n')
            r.set("private_key_path", new_file_path.replace("\\", "/"))
        elif file_type == "cert" and line.startswith("cert_path"):
            updated_lines.append(f'cert_path = "{path_for_env}"\n')
            r.set("cert_path", new_file_path.replace("\\", "/"))
        else:
    ...
    return new_file_path.replace("\\", "/")  # Return the updated relative path
