...
# Connect to Redis
redis_client = redis.Redis(host=QUEUE_HOST, port=QUEUE_PORT, db=0, decode_responses=True)
...
class CustomRotatingFileHandler(RotatingFileHandler):
    ...
    def doRollover(self):
        ...
        if new_date != self.current_date:
            self.current_date = new_date
            self.base_filename = self.base_filename.split("_")[0] + f"_{new_date}.txt"

        for i in range(self.backupCount - 1, 0, -1):
            src = f"logs/system_logs_{i}_{self.current_date}.txt"
            dst = f"logs/system_logs_{i+1}_{self.current_date}.txt"
        ...
        if os.path.exists(self.base_filename):
            try:
                shutil.move(self.base_filename, f"logs/system_logs_1_{self.current_date}.txt")
            except Exception as e:
                print(f"Failed to rotate {self.base_filename}: {e}")
...
def configure_logging(max_bytes, backup_count):
    ...
    log_handler = CustomRotatingFileHandler("logs/system_logs.txt", maxBytes=max_bytes, backupCount=backup_count)
