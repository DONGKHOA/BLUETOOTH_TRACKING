...
#ifndef __EDDYSTONE_PROTOCOL_H__
#define __EDDYSTONE_PROTOCOL_H__
...
#define EDDYSTONE_UID_DATA_LEN \
  (EDDYSTONE_UID_RANG_DATA_LEN + EDDYSTONE_UID_NAMESPACE_LEN + EDDYSTONE_UID_INSTANCE_LEN)
// TLM
...
#define EDDYSTONE_TLM_DATA_LEN                                     \
  (EDDYSTONE_TLM_VERSION_LEN + EDDYSTONE_TLM_BATTERY_VOLTAGE_LEN + \
...
#define EDDYSTONE_URL_MAX_LEN \
  (EDDYSTONE_URL_SCHEME_LEN + EDDYSTONE_URL_ENCODED_MAX_LEN)
...
typedef struct
{
  ...
  uint8_t type;
  uint8_t flags;
} __attribute__((packed)) eddystone_flags_t;
...
} __attribute__((packed)) eddystone_uuid_t;
...
} __attribute__((packed)) eddystone_frame_t;

/* eddystone packet type */
typedef struct
{
  eddystone_flags_t flags;
  eddystone_uuid_t  uuid;
  eddystone_frame_t frame;
} __attribute__((packed)) eddystone_packet_t;
 ...
 *
 */
static inline bool eddystone_is_char_invalid(int ch)
{
  return (ch >= 0x00 && ch <= 0x20) || (ch >= 0x7f && ch <= 0xff);
}

#endif /* __EDDYSTONE_PROTOCOL_H__ */
