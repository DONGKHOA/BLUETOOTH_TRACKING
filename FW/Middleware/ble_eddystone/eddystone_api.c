...
static esp_err_t eddystone_uid_received(const uint8_t* buf, uint8_t len, eddystone_result_t* res);
static esp_err_t eddystone_url_received(const uint8_t* buf, uint8_t len, eddystone_result_t* res);
static char* eddystone_resolve_url_scheme(const uint8_t* url_start, const uint8_t* url_end);
static esp_err_t eddystone_tlm_received(const uint8_t* buf, uint8_t len, eddystone_result_t* res);
static esp_err_t eddystone_get_inform(const uint8_t* buf, uint8_t len, eddystone_result_t* res);
...
static char* eddystone_resolve_url_scheme(const uint8_t *url_start, const uint8_t *url_end)
{
    ...
    for (; p <= url_end; p++) {
        if (eddystone_is_char_invalid((*p))) {
...
static esp_err_t eddystone_url_received(const uint8_t* buf, uint8_t len, eddystone_result_t* res)
{
    ...
    url_res = eddystone_resolve_url_scheme(buf+pos, buf+len-1);
...
static esp_err_t eddystone_get_inform(const uint8_t* buf, uint8_t len, eddystone_result_t* res)
{
    ...
    switch(res->common.frame_type)
    {
        case EDDYSTONE_FRAME_TYPE_UID: {
            ret = eddystone_uid_received(buf, len, res);
            break;
        }
        case EDDYSTONE_FRAME_TYPE_URL: {
            ret = eddystone_url_received(buf, len, res);
            break;
        }
        case EDDYSTONE_FRAME_TYPE_TLM: {
            ret = eddystone_tlm_received(buf, len, res);
            break;
        }
        default:
            break;
...
esp_err_t eddystone_decode(const uint8_t* buf, uint8_t len, eddystone_result_t* res)
{
    ...
    return eddystone_get_inform(buf+pos, len-pos, res);
}
