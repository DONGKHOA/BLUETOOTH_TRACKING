...
void APP_BLE_TRACKING_CreateTask(void)
{
}

void APP_BLE_TRACKING_Init(void)
{
  esp_err_t status;
  ...
  if ((status = esp_ble_gap_register_callback(APP_BLE_TRACKING_GAP_Callback)) != ESP_OK)
  {
  ...
  case ESP_GAP_BLE_SCAN_RESULT_EVT:
  {
    ...
    case ESP_GAP_SEARCH_INQ_RES_EVT:
    {
      ...
      if (ret)
      {
        ...
        // eddystone frame packet. just return
        return;
      }
      else
      {
        // The received adv data is a correct eddystone frame packet.
        // Here, we get the eddystone information in eddystone_res, we can use
        // the data in res to do other things. For example, just print them:
        ESP_LOGI(TAG, "--------Eddystone Found----------");
        esp_log_buffer_hex("EDDYSTONE_DEMO: Device address:",
                           scan_result->scan_rst.bda, ESP_BD_ADDR_LEN);

        ESP_LOGI(TAG, "RSSI RAW: %d dbm\n", scan_result->scan_rst.rssi);
      }
      break;
    }
    default:
