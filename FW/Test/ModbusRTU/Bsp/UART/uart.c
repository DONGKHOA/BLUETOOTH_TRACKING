...
void IRAM_ATTR
UART_RX_ISR (void *arg)
{
  EXTI_ISR_Handler(arg);
}
...
BSP_uartDriverInit (uart_port_num_t e_uart_port)
{
  ...
  uart_driver_install(e_uart_port, RX_BUF_SIZE * 2, 0, 0, NULL, ESP_INTR_FLAG_IRAM);
  uart_param_config(e_uart_port, &uart_config);
}

void
BSP_uartIntrInit (gpio_num_t      e_rx_pin,
                  gpio_int_type_t e_intr_type,
                  exti_callback_t callback)
{
  EXTI_Init(e_rx_pin, e_intr_type, callback);
}
