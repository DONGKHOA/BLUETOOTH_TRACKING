 ...
 */
#define AWS_IOT_PASSWORD_ALPN_LENGTH \
  ((uint16_t)(sizeof(AWS_IOT_PASSWORD_ALPN) - 1))
 ...
 */
#define MQTT_EXAMPLE_MESSAGE_LENGTH \
  ((uint16_t)(sizeof(MQTT_EXAMPLE_MESSAGE) - 1))
 ...
 */
#define METRICS_STRING                                                       \
  "?SDK=" OS_NAME "&Version=" OS_VERSION "&Platform=" HARDWARE_PLATFORM_NAME \
  "&MQTTLib=" MQTT_LIB
...
/**
 * @brief Append the username with the metrics string if #CLIENT_USERNAME is
 * defined.
 *
 ...
 * @param[out] pBrokerSessionPresent Session was already present in the broker
 * or not. Session present response is obtained from the CONNACK from broker.
 *
 ...
 */
int
aws_iot_demo_main (int argc, char **argv)
{
  ...
  /* Seed pseudo random number generator (provided by ISO C standard library)
   * for use by retry utils library when retrying failed network operations. */
  ...
  if (returnStatus == EXIT_SUCCESS)
  {
    for (;;)
    {
       ...
       * a timeout. Timeout value will be exponentially increased till the
       * maximum attempts are reached or maximum timeout value is reached. The
       * function returns EXIT_FAILURE if the TCP connection cannot be
       * established to broker after configured number of attempts. */
...
static int
connectToServerWithBackoffRetries (NetworkContext_t *pNetworkContext,
                                   MQTTContext_t    *pMqttContext,
                                   bool             *pClientSessionPresent,
                                   bool             *pBrokerSessionPresent)
{
  ...
  pNetworkContext->xTlsContextSemaphore
      = xSemaphoreCreateMutexStatic(&xTlsContextSemaphoreBuffer);
  ...
  pNetworkContext->pcServerRootCASize
  ...
  do
  {
    ...
    if (tlsStatus == TLS_TRANSPORT_SUCCESS)
    {
      ...
      returnStatus = establishMqttSession(
          pMqttContext, createCleanSession, pBrokerSessionPresent);
    ...
    if (returnStatus == EXIT_FAILURE)
    {
      ...
      {
        LogWarn(
            ("Connection to the broker failed. Retrying connection "
             "after %hu ms backoff.",
...
static int
getNextFreeIndexForOutgoingPublishes (uint8_t *pIndex)
{
...
static void
cleanupOutgoingPublishAt (uint8_t index)
{
  assert(outgoingPublishPackets != NULL);
...
static void
cleanupOutgoingPublishWithPacketID (uint16_t packetId)
{
  uint8_t index = 0;
...
static int
handlePublishResend (MQTTContext_t *pMqttContext)
{
  int               returnStatus     = EXIT_SUCCESS;
  ...
  while (packetIdToResend != MQTT_PACKET_ID_INVALID)
  {
    ...
    {
      LogError(
          ("Packet id %u requires resend, but was not found in "
...
static void
handleIncomingPublish (MQTTPublishInfo_t *pPublishInfo,
                       uint16_t           packetIdentifier)
{
...
static void
updateSubAckStatus (MQTTPacketInfo_t *pPacketInfo)
{
...
static int
handleResubscribe (MQTTContext_t *pMqttContext)
{
  int                       returnStatus     = EXIT_SUCCESS;
  MQTTStatus_t              mqttStatus       = MQTTSuccess;
  BackoffAlgorithmStatus_t  backoffAlgStatus = BackoffAlgorithmSuccess;
  ...
  do
  {
     ...
     * Note: reusing the value specified in globalSubscribePacketIdentifier is
     * acceptable here because this function is entered only after the receipt
     * of a SUBACK, at which point its associated packet id is free to use. */
     ...
     * globalSubAckStatus is updated in eventCallback to reflect the status of
     * the SUBACK sent by the broker. It represents either the QoS level granted
     * by the server upon subscription, or acknowledgement of server rejection
     * of the subscription request. */
    if (globalSubAckStatus == MQTTSubAckFailure)
    {
      /* Generate a random number and get back-off value (in milliseconds) for
       * the next re-subscribe attempt. */
      backoffAlgStatus = BackoffAlgorithm_GetNextBackoff(
          &retryParams, generateRandomNumber(), &nextRetryBackOff);
      ...
      {
        LogWarn(
            ("Server rejected subscription request. Retrying "
...
static void
eventCallback (MQTTContext_t          *pMqttContext,
               MQTTPacketInfo_t       *pPacketInfo,
               MQTTDeserializedInfo_t *pDeserializedInfo)
{
  uint16_t packetIdentifier;
  ...
  else
  {
    ...
    switch (pPacketInfo->type)
    {
      case MQTT_PACKET_TYPE_SUBACK:
         ...
         * single topic requested. The SUBACK will be parsed to obtain the
         * status code, and this status code will be stored in global variable
         * globalSubAckStatus. */
        updateSubAckStatus(pPacketInfo);

        /* Check status of the subscription request. If globalSubAckStatus does
         * not indicate server refusal of the request (MQTTSubAckFailure), it
         * contains the QoS level granted by the server, indicating a successful
...
static int
establishMqttSession (MQTTContext_t *pMqttContext,
                      bool           createCleanSession,
                      bool          *pSessionPresent)
{
  int               returnStatus = EXIT_SUCCESS;
...
static int
disconnectMqttSession (MQTTContext_t *pMqttContext)
{
  MQTTStatus_t mqttStatus   = MQTTSuccess;
  int          returnStatus = EXIT_SUCCESS;
...
static int
subscribeToTopic (MQTTContext_t *pMqttContext)
{
  int          returnStatus = EXIT_SUCCESS;
  MQTTStatus_t mqttStatus;
  ...
  (void)memset(
      (void *)pGlobalSubscriptionList, 0x00, sizeof(pGlobalSubscriptionList));
...
static int
unsubscribeFromTopic (MQTTContext_t *pMqttContext)
{
  int          returnStatus = EXIT_SUCCESS;
  MQTTStatus_t mqttStatus;
...
static int
publishToTopic (MQTTContext_t *pMqttContext)
{
  int          returnStatus = EXIT_SUCCESS;
  MQTTStatus_t mqttStatus   = MQTTSuccess;
...
static int
initializeMqtt (MQTTContext_t *pMqttContext, NetworkContext_t *pNetworkContext)
{
  int                  returnStatus = EXIT_SUCCESS;
  MQTTStatus_t         mqttStatus;
  ...
  mqttStatus = MQTT_Init(
      pMqttContext, &transport, Clock_GetTimeMs, eventCallback, &networkBuffer);

  if (mqttStatus != MQTTSuccess)
  {
    returnStatus = EXIT_FAILURE;
    LogError(
        ("MQTT_Init failed: Status = %s.", MQTT_Status_strerror(mqttStatus)));
  }
  else
  {
    ...
    {
      returnStatus = EXIT_FAILURE;
      LogError(("MQTT_InitStatefulQoS failed: Status = %s.",
                MQTT_Status_strerror(mqttStatus)));
    }
  }
...
static int
subscribePublishLoop (MQTTContext_t *pMqttContext)
{
  int            returnStatus    = EXIT_SUCCESS;
  MQTTStatus_t   mqttStatus      = MQTTSuccess;
  ...
  if (returnStatus == EXIT_SUCCESS)
  {
     ...
     * ( SUBACK ) will be received here. However after sending the subscribe,
     * the client may receive a publish before it receives a subscribe ack.
     * Since this demo is subscribing to the topic to which no one is
     * publishing, probability of receiving publish message before subscribe ack
     * is zero; but application must be ready to receive any packet. This demo
     * uses MQTT_ProcessLoop to receive packet from network. */
    returnStatus = waitForPacketAck(pMqttContext,
                                    globalSubscribePacketIdentifier,
                                    MQTT_PROCESS_LOOP_TIMEOUT_MS);
  }

  /* Check if recent subscription request has been rejected. globalSubAckStatus
   * is updated in eventCallback to reflect the status of the SUBACK sent by the
   * broker. */
  if ((returnStatus == EXIT_SUCCESS)
      && (globalSubAckStatus == MQTTSubAckFailure))
  {
    ...
    LogInfo(
        ("Server rejected initial subscription request. Attempting to "
         "re-subscribe to topic %.*s.",
         MQTT_EXAMPLE_TOPIC_LENGTH,
         MQTT_EXAMPLE_TOPIC));
  ...
  if (returnStatus == EXIT_SUCCESS)
  {
    ...
    for (publishCount = 0; publishCount < maxPublishCount; publishCount++)
    {
       ...
       * ping responses. */
      mqttStatus
          = processLoopWithTimeout(pMqttContext, MQTT_PROCESS_LOOP_TIMEOUT_MS);
  ...
  /* Reset global SUBACK status variable after completion of subscription
   * request cycle. */
  globalSubAckStatus = MQTTSubAckFailure;
...
static int
waitForPacketAck (MQTTContext_t *pMqttContext,
                  uint16_t       usPacketIdentifier,
                  uint32_t       ulTimeout)
{
  ...
  {
    LogError(
        ("MQTT_ProcessLoop failed to receive ACK packet: Expected ACK Packet "
         "ID=%02" PRIx16 ", LoopDuration=%" PRIu32 ", Status=%s",
         usPacketIdentifier,
...
static MQTTStatus_t
processLoopWithTimeout (MQTTContext_t *pMqttContext, uint32_t ulTimeoutMs)
{
  uint32_t ulMqttProcessLoopTimeoutTime;
  uint32_t ulCurrentTime;
