...
#define RX_BUF_SIZE 1024
  ...
  /**
   * The function initializes a UART driver with specified configurations such
   * as baud rate, data bits, parity, flow control, and stop bits.
   *
   * @param u32_uart_port The `u32_uart_port` parameter specifies which UART
   * port to initialize. It could be something like `UART_NUM_0` or `UART_NUM_1`
   ...
   * interface and is responsible for sending data out from the microcontroller
   * to another device.
   * @param e_rx_pin The `e_rx_pin` parameter in the `uartDriverInit` function
   * is the GPIO pin number to which the receive (RX) pin of the UART interface
   * is connected. This pin is used for receiving data from an external device
   * or another UART transmitter.
   ...
   * UART communication.
   * @param e_data_bit The `e_data_bit` parameter in the `uartDriverInit`
   * function specifies the number of data bits per frame. It determines the
   * size of the data being transmitted or received over the UART communication.
   * The options for `data_bit` typically include values like
   * `UART_DATA_5_BITS`, `UART_DATA
   ...
   * @param e_stop_bit The `e_stop_bit` parameter in the `uartDriverInit`
   * function refers to the number of stop bits used in UART communication. It
   * specifies the number of bits that signal the end of a data frame. Common
   * values for `stop_bit` include `UART_STOP_BITS_1` for one stop bit
   */
   ...
   *
   * @param u32_uart_port The `uart_port` parameter is the UART port or
   * interface through which the data will be sent. It is typically a
   * hardware-specific identifier that specifies which UART peripheral to use
   * for communication.
   ...
   *
   * This function waits until the UART transmission is finished or the
   * specified timeout period elapses.
   *
   * @param u32_uart_port The UART port number to wait for the transmission to
   * complete.
   * @param u32_timeout The maximum time to wait for the transmission to
