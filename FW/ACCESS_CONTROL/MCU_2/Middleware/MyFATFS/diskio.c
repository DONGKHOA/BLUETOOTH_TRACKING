...
{
  Stat = STA_NOINIT;
  if (DEV_SDCard_Init(&spi_sdcard_handle, SPI3_HOST, SPI3_CS_SD_CARD_PIN) == MSD_OK)
  {
    Stat &= ~STA_NOINIT;
  }
  return Stat;
...
)
{
  ...
  if (DEV_SDCard_ReadBlock(
          spi_sdcard_handle, (uint8_t *)buff, (uint32_t)(sector), count, SPI3_CS_SD_CARD_PIN)
      == MSD_OK)
  {
    res = RES_OK;
  }
...
DRESULT
disk_ioctl(BYTE  pdrv, /* Physical drive number (0..) */
