...
void
DECODE_Command (char *json_string, char *command)
{
  ...
  {
    strcpy(command, cmd_item->valuestring); // UNSAFE - caller must ensure
                                            // buffer is large enough
  }
  else
  {
    command[0] = '\0'; // Empty string on failure
  }
...
// void
// DECODE_User_Data (char *json_str, int *user_id, char **user_name, int
// user_len)
// {
