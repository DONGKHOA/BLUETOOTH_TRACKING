...
extern "C"
{
   ...
   * representing the GPIO pin number on a microcontroller. It is used to
  specify the GPIO pin for which
   * the input configuration needs to be initialized.
   * @param e_gpio_res_pull The `e_gpio_res_pull` parameter is used to specify
  the pull configuration for the
   * GPIO pin. It can have two values:
   */
  void BSP_gpioInputInit(gpio_num_t       e_gpio_pin,
   ...
   *
   * @param e_gpio_pin The `e_gpio_pin` parameter is the GPIO pin number that
   * you want to initialize as an output pin.
   */
